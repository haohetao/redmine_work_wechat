        ...
        def login_with_login_dingtalk
          ...
          if auth_code.blank?
            ...
            if state == "STATE"
              ...
              url = URI.parse(apiUrl + "/sns/getuserinfo_bycode?accessKey=#{appid}&timestamp=#{timestamp}&signature=#{signature}")
            ...
            begin
              ...
              uri = URI.parse(apiUrl + "/user/getuserinfo?access_token=#{token}&code=#{auth_code}")
              http = Net::HTTP.new(uri.host, uri.port)
              http.use_ssl = true
              request = Net::HTTP::Get.new(uri.request_uri)
              response = http.request(request)
